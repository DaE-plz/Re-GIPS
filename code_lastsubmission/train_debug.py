...
dataset = projection_dataset.ProjectionTensorDataset(ap_dir='G:/dataset/projection_ap', lt_dir='G:/dataset/projection_lt')
...
if os.path.exists(checkpoint_path):
    ...
    while epoch < 200:
        ...
        for i, (ap_truth, lt_truth) in enumerate(tqdm.tqdm(train_loader)):
            ...
            print(f"Shape of ap_truth: {ap_truth.shape}")
            print(f"Shape of lt_truth: {lt_truth.shape}")

            geomery_features = dlgips_geometry_encoder(ap_truth)  # f_src_g
            texture_features = dlgips_texture_encoder(ap_truth)  # f_t
            print(f"Shape of geometry_features: {geomery_features.shape}")
            print(f"Shape of texture_features: {texture_features.shape}")
            ...
            print(f"Shape of transformed_features_ap: {transformed_features_ap.shape}")
            ...
            print(f"Shape of generated_image_ap: {generated_image_ap.shape}")
            print(f"Shape of generated_image_lt: {generated_image_lt.shape}")
            print(f"Shape of generated_original_image: {generated_original_image}")
            ...
            print(f"Shape of generated_ap_feature: {generated_ap_feature}")
            print(f"Shape of generated_lt_feature: {generated_lt_feature}")
            ...
            if i % 1 == 0:  # 每1个batch保存一次图像
                ...
                # 保存输入geometry特征
                save_as_tif(geomery_features[0].cpu().detach(), f'{save_path}/{epoch}_{i}_geometry_features.tif')

                # 保存输出（变换后）geometry特征
                save_as_tif(transformed_features[0].cpu().detach(), f'{save_path}/{epoch}_{i}_transformed_features.tif')
            # 累计损失和批次数量
