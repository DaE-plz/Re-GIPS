...
def calculate_metrics(pred, truth):
    ...
    rmse = np.sqrt(mse_loss(pred, truth, reduction='mean').item())

    # # Calculate SSIM
    # ssim_index = ssim(truth_np, pred_np, data_range=truth_np.max() - truth_np.min())
    ssim_index=0
    # Calculate PSNR
...
with torch.no_grad():
    for idx, (ap_truth, lt_truth) in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        # # Apply the intensity window to all images
        # #print("Image dimensions:lt", lt_truth.shape,"and generated_lt:", generated_image_lt.shape)
        #
        # ap_truth_windowed = apply_window(ap_truth, window_level, window_width)
        ...
        #print("generated_image_lt_windowed:",generated_image_lt_windowed.shape )
        # Save the windowed images as tif files
        # save_as_tif(ap_truth_windowed, os.path.join(save_path, f'ap_truth_{idx}_windowed.tif'))
        ...
        # 计算指标
        mae, rmse, ssim_index, psnr_value = calculate_metrics(generated_image_ap, ap_truth)
        # 记录指标
