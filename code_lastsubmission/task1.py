...
if __name__ == '__main__':
    ...
    dataset = projection_dataset.ProjectionTensorDataset(
        ap_dir='G:/dataset/projection_ap', lt_dir='G:/dataset/projection_lt'
    )
    ...
    for idx, (ap_truth, _) in enumerate(data_loader):  # Ignore lt_truth for this verification
        if idx == 2:
            ap_truth = ap_truth.to(device)

            # Backward projection to get the volume from AP projections
            volume = ap_ap_operators.backward_project(ap_truth)

            # Here you could include a refinement step if you have a model for it
            # volume = refine_volume(volume)

            # Forward projection to simulate AP projection from the reconstructed volume
            projection = ap_aplt_operators.forward_project(volume)

            print(projection.shape)
            # Splitting the projections into AP and LT projections
            ap_pro = projection[:, 0, :, :].squeeze()  # Assuming the AP projection is the first slice
            lt_pro = projection[:, 1, :, :].squeeze()  # Assuming the LT projection is the second slice

            # Save projections as TIFF files
            tiff.imwrite('G:/dataset/output/ap_projection.tif', ap_pro.cpu().numpy())
            tiff.imwrite('G:/dataset/output/lt_projection.tif', lt_pro.cpu().numpy())

            print("AP and LT projections saved as TIFF files.")
