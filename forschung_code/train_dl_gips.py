...
# 定义优化器和超参数
optimizer_geometry_encoder = torch.optim.Adam(dlgips_geometry_encoder.parameters(), lr=0.0001,betas=(0.5, 0.999))
optimizer_texture_encoder = torch.optim.Adam(dlgips_texture_encoder.parameters(), lr=0.0001,betas=(0.5, 0.999))

optimizer_geometry_transform = torch.optim.Adam(dlgips_geometry_transform.parameters(), lr=0.0001,betas=(0.5, 0.999))
optimizer_image_generator = torch.optim.Adam(dlgips_image_generator.parameters(), lr=0.0001,betas=(0.5, 0.999))
optimizer_image_discriminator = torch.optim.Adam(dlgips_image_discriminator.parameters(), lr=0.0001,betas=(0.5, 0.999))
...
if os.path.exists(checkpoint_path):
    ...
    while epoch < 1000:
        ...
        for i, (ap_truth, lt_truth) in enumerate(tqdm.tqdm(train_loader)):
            ...
            geometry_feature_lt_truth=dlgips_geometry_encoder(lt_truth)
            ...
            # 计算对抗性损失
            batch_size = lt_truth.size(0)  # 获取批次大小

            # 对于每个尺度的输出，创建匹配尺寸的目标标签
            ...
            loss_adv1 = adversarial_loss.adversarial_loss(real_outputs1, fake_outputs1, real_labels1.to(device), fake_labels1.to(device))
            loss_adv2 = adversarial_loss.adversarial_loss(real_outputs2, fake_outputs2, real_labels2.to(device),
                                                          fake_labels2.to(device))
            loss_adv3 = adversarial_loss.adversarial_loss(real_outputs3, fake_outputs3, real_labels3.to(device),
                                                          fake_labels3.to(device))
            ...
            if i == len(train_loader) - 1:
                ...
                _generated_image_lt = generated_image_lt[0].cpu().detach()
                _generated_image_original = generated_original_image[0].cpu().detach()
                save_as_tif(_generated_image_ap, f'{save_path}/{epoch}_generated_ap.tif')
                save_as_tif(_generated_image_lt, f'{save_path}/{epoch}_generated_lt.tif')
                save_as_tif(_generated_image_original, f'{save_path}/{epoch}_generated_original.tif')

                # 保存输入geometry特征
                save_as_tif(geomery_features[0].cpu().detach(), f'{save_path}/{epoch}_{i}_geometry_features_ap.tif')

                # 保存lt角度的特征
                save_as_tif(geometry_feature_lt_truth[0].cpu().detach(), f'{save_path}/{epoch}_{i}_geometry_features_lt_truth.tif')

                # 保存输出（变换后）ap geometry特征
                save_as_tif(transformed_features_ap[0].cpu().detach(), f'{save_path}/{epoch}_{i}_transformed_features_ap.tif')
                # 保存输出（变换后）lt geometry特征
                save_as_tif(transformed_features_lt[0].cpu().detach(),
                            f'{save_path}/{epoch}_{i}_transformed_features_lt.tif')
            # 累计损失和批次数量
        ...
        with torch.no_grad():
            ...
            for ap_truth, lt_truth in val_loader:
                ...
                loss_adv1 = adversarial_loss.adversarial_loss(real_outputs1, fake_outputs1, real_labels1.to(device),
                                                              fake_labels1.to(device))
                loss_adv2 = adversarial_loss.adversarial_loss(real_outputs2, fake_outputs2, real_labels2.to(device),
                                                              fake_labels2.to(device))
                loss_adv3 = adversarial_loss.adversarial_loss(real_outputs3, fake_outputs3, real_labels3.to(device),
                                                              fake_labels3.to(device))
